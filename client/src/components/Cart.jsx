...
const MyCart = () => {
  ...
  useEffect(() => {
    ...
    getMyCart();//here above fun is called
  ...
  const retrieveMyCart = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/users/cart/mycart?userId=${user}`
      );
  ...
  const deleteProductFromCart = async (cartId, e) => {
    try {
      const response = await axios.delete(
        `http://localhost:8080/users/cart/mycart/remove?cartId=${cartId}`
      );
  ...
  const handleNavigate = () => {
    // Data to pass
    const data = myCartData;
    
    // Navigate to '/destination' and pass state
                ...
                {myCartData.map((cartData) => {
                  return (
                    <tr>
                      ...
                      <td>
                        <b>{cartData.watchName}</b>
                      </td>
                      <td>
                        <b>{cartData.watchDescription}</b>
                      </td>
                      <td>
